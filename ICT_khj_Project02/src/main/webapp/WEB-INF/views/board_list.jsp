				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="k" items="${board_list}" varStatus="vs">
							<tr>
							    <td>${paging.totalRecord - ((paging.nowPage-1)*paging.numPerPage + vs.index )} ${vs.index}</td>
							    ...
							    <c:choose>
							    	...
							    	<c:otherwise>
							    		<a href="board_detail.do?board_idx=${k.board_idx}&cPage=${paging.nowPage}">${k.title }</a></td>
							    	</c:otherwise>
							    </c:choose>
							    <td>${k.writer }</td>
