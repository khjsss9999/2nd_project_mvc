...
<body>
			...
			<div class="insert">
				...
				<table>
					...
					<tr>
						...
						<td class="userin"><input type="text" name="user_f_email"> 
						<span class="a">@</span> <input type="text" id="domain-txt" name="user_b_email"> 
	...
	function check_pw(){
		if($('#pw').val() !='' && $('#pwd').val() != ''){
		    if($('#pw').val() == $('#pwd').val()){
		        $('#check').text('비밀번호가 일치합니다.').css('color', 'green')
		    }else{
		        $('#check').text('비밀번호가 일치하지 않습니다.').css('color', 'red')
		    }
		}
}
	</script>
